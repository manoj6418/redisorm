...
test:
	$(MAKE) docker.start
	@if [ -z "$(RORM_VERSION)" ]; then \
		echo "RORM_VERSION not set, running all tests"; \
		$(MAKE) test.ci; \
	else \
		MAJOR_VERSION=$$(echo "$(RORM_VERSION)" | cut -d. -f1); \
		if [ "$$MAJOR_VERSION" -ge 8 ]; then \
			echo "RORM_VERSION $(RORM_VERSION) >= 8, running all tests"; \
			$(MAKE) test.ci; \
		else \
			echo "RORM_VERSION $(RORM_VERSION) < 8, skipping vector_sets tests"; \
